...
type Machine interface {
	...
	Command(a Applier, conn redcon.Conn, cmd redcon.Command, index uint64) (interface{}, error)
...
func (n *Node) doCommand(conn redcon.Conn, cmd redcon.Command, index uint64) (interface{}, error) {
	if len(cmd.Args) == 0 {
		return nil, nil
	}
	var val interface{}
	...
	default:
		val, err = n.handler.Command((*nodeApplier)(n), conn, cmd, index)
		if err == ErrDisabled {
...
func (m *nodeFSM) Apply(l *raft.Log) interface{} {
	...
	val, err := (*Node)(m).doCommand(nil, cmd, l.Index)
	if err != nil {
		return err
	}
	return val
